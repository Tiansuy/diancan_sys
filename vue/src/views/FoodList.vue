<template>
    <div>
        <div class="menu">
          <div @click="chooseFoodType(item.food_type_name)" v-for="(item,index) in menuList" :key="index" class="item" >
            <div
              v-if="index==choosed"
        ...
        <div v-for="(item,i) in tableData" :key="i" class="food-item">
            ...
            <el-input-number v-model="item.select_num" @change="handleChange(item)" :min="0" :max="10" label="描述文字"></el-input-number>
        </div>
        <div class="total">
            ...
            <el-dialog title="购物车" v-model="dialogTableVisible" top="20vh" width="40%">
                <div v-for="(item,i) in selectItems" :key="i" style="display:flex; height:42px;border:1px solid #00000020;margin-top:1px;">
                    ...
                    <el-input-number style="flex:0.2;" size="mini" v-model="item.select_num" @change="handleChange(item)" :min="0" :max="10" label="描述文字"></el-input-number>
                </div>
            ...
            <div @click="chooseTable" class="total-right">
                ...
                <el-dialog title="选择桌号" v-model="submitDialog" center top="20vh" width="30%">
                    <div style="margin-left:30%;"><el-input-number v-model="table_number" size="medium" :min="1" :max="30" label="描述文字"></el-input-number></div>
                    <template #footer>
    ...
    methods:{
      ...
      chooseFoodType(foodType){
        ...
        save(){
          ...
          this.form = {
            uploader:"周杰伦",
            bill_date:FDate.getFormatDate(),
            bill_price:this.getSelectionTotalPraice(),
          ...
          request.post("bill/add-bill",this.form).then(res=>{
            ...
            if (res.code === '0') {
              ...
              for(let i=0;i<that.selectItems.length;i++){
                ...
                request.post("item",that.form).then(item_res=>{
                  console.log("item-res:",item_res)
                  if(i==that.selectItems.length-1){
                    this.$message({
                      type: "success",
                      message: "添加成功"
                    })
